...
bool do_test(const Landscape & landscape, const RestorationPlan<Landscape>& plan, int seed) {
    ...
    for(int percent_budget : {10, 20, 30}) {
        ...
        const double glutton_inc_value = ECA::get().eval_solution(landscape, *glutton_inc_solution);
        const double glutton_dec_value = ECA::get().eval_solution(landscape, *glutton_dec_solution);
        const double pl_3_value = ECA::get().eval_solution(landscape, *pl_3_solution);
