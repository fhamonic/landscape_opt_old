...
inline std::ostream & operator<<(std::ostream & os,
                                 const LinearExpression & linear_expr) {
    ranges::for_each(
        ranges::view::zip(linear_expr.getCoefficients(),
                          linear_expr.getIndices()),
        [&os](const auto p) { os << p.first << "*x" << p.second << " + "; });
    return os << linear_expr.getConstant();
}
inline std::ostream & operator<<(std::ostream & os,
                                 const QuadraticExpression & quad_expr) {
    ranges::for_each(ranges::view::zip(quad_expr.getQuadCoefficients(),
                                       quad_expr.getQuadIndices1(),
                                       quad_expr.getQuadIndices2()),
                     [&os](const auto p) {
                         os << std::get<0>(p) << "*x" << std::get<1>(p) << "x"
                            << std::get<2>(p) << " + ";
                     });
    return os << quad_expr.getLineraExpression();
}
inline std::ostream & operator<<(std::ostream & os,
                                 linear_ineq_constraint constraint) {
    return os << constraint.linear_expression
              << (constraint.sense == LESS ? " <= 0" : " >= 0");
}
inline std::ostream & operator<<(std::ostream & os,
                                 linear_range_constraint constraint) {
    return os << constraint.lower_bound
              << " <= " << constraint.linear_expression
              << " <= " << constraint.upper_bound;
}
inline std::ostream & operator<<(std::ostream & os,
                                 quadratic_ineq_constraint constraint) {
