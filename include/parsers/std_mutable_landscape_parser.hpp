...
class StdMutableLandscapeParser : public concepts::Parser<MutableLandscape> {
    ...
    public:
        static StdMutableLandscapeParser & get() noexcept {
            return singleton;
        }
        ~StdMutableLandscapeParser();
   
        MutableLandscape parse(const std::filesystem::path file_name);
        void write(const MutableLandscape & landscape, const std::filesystem::path output, const std::string name, bool use_range_ids=true);
};
