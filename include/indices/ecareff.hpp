...
public:
    ...
    typename LS::GR::template NodeMap<
        typename LS::GR::template NodeMap<double> *> *
    getReff(const GR & g, const DM & resistance);
...
typename GR::template NodeMap<typename GR::template NodeMap<double> *> *
ECAReff::getReff(const GR & g, const DM & resistance) {
    ...
    const int nb_connected_components =
        lemon::connectedComponents(g, componentMap);
    ...
    for(int i = 0; i < nb_connected_components; i++) indices_count[i] = 0;
    ...
    for(typename Graph::ArcIt e(g); e != lemon::INVALID; ++e) {
        ...
        if(component != componentMap[v]) continue;

        const int indice_u = indiceMap[u];
        const int indice_v = indiceMap[v];

        L[component](indice_u, indice_v) = L[component](indice_v, indice_u) =
            -1 / resistance[e];
    ...
    for(typename Graph::NodeIt u(g); u != lemon::INVALID; ++u) {
        ...
        for(typename Graph::NodeIt v(g); v != lemon::INVALID; ++v) {
            ...
            (*(*effective_resistance)[u])[v] =
                L_plus[component](indice_u, indice_u) +
                L_plus[component](indice_v, indice_v) -
                2 * L_plus[component](indice_u, indice_v);
        }
    }
...
template <typename LS>
double ECAReff::eval(const LS & landscape) {
    using Graph = typename LS::Graph;
    using QualityMap = typename LS::QualityMap;
    using DifficultyMap = typename LS::ProbabilityMap;
    using ResistancesMap =
        typename GR::template NodeMap<typename GR::template NodeMap<double> *>;
    ...
    for(typename Graph::NodeIt s(g); s != lemon::INVALID; ++s) {
        for(typename Graph::NodeIt t(g); t != lemon::INVALID; ++t) {
            if(graph.id(s) >= graph.id(t)) continue;

            const double r_st = (*(*effective_resistance)[s])[t];

            // std::cout << "r( " << g.id(s) << " , " << g.id(t) << " ) = " <<
            // r_st << std::endl;
