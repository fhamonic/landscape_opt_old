...
int main() {
    ...
    for(int i = 0; i < 200; ++i)
        orig_values.emplace_back(x_dis(gen), y_dis(gen));
            ...
            for(double median : median_values) {
                for(Point orig : orig_values) {
                    Instance instance = make_instance_quebec(
                        pow, thresold, median, orig, Point(32360, 20000));
                    ...
                    if(lemon::countNodes(landscape.getNetwork()) > 100)
                        continue;
                    ...
                    Helper::printInstance(landscape, plan,
                                          "quebec-(" + std::to_string(orig.x) +
                                              "," + std::to_string(orig.y) +
                                              ").eps");

                    for(double budget_percent : budget_percent_values) {
                        const double budget =
                            (budget_percent * plan.totalCost()) / 100;
                        ...
                        //     bogo.setSeed(i);
                        //     Solution solution = bogo.solve(landscape, plan,
                        //     budget); total_bogo_time +=
                        //     solution->getComputeTimeMs(); total_bogo_cost +=
                        //     solution->getCost(); total_bogo_eca +=
                        //     std::pow(ECA().eval(Helper::decore_landscape(landscape,
                        //     plan, solution)), 2);
                        // }
                        // data_log << pow << " "
                        //             << thresold << " "
                        ...
                        for(const std::unique_ptr<concepts::Solver> & solver :
                            solvers) {
                            Solution solution =
                                solver.get()->solve(landscape, plan, budget);

                            const double total_eca =
                                std::pow(ECA().eval(Helper::decore_landscape(
                                             landscape, plan, solution)),
                                         2);

                            data_log
                                << pow << " " << thresold << " " << median
                                << " " << orig << " " << budget_percent << " "
                                << budget << " " << solver->toString() << " "
                                << solution.getComputeTimeMs() << " "
                                << solution.getCost() << " " << total_eca << " "
                                << std::endl;
                        }
                    }
                }
    ...
    std::cout << "min_arcs " << min_arcs << std::endl
              << "max_arcs " << max_arcs << std::endl;
