...
static void populate(std::list<concepts::Solver *> & solvers) {
    ...
    Solvers::Randomized_Rounding_ECA * randomized_rounding =
        new Solvers::Randomized_Rounding_ECA();
...
static void clean(std::list<concepts::Solver *> & solvers) {
    for(concepts::Solver * solver : solvers) delete solver;
}
...
int main(int argc, const char * argv[]) {
    ...
    populate(solvers);
    if(argc < 3) {
        std::cerr << "input requiered : <worst case dir> <worst case name>"
                  << std::endl;
        clean(solvers);
        return EXIT_FAILURE;
    }
    ...
    MutableLandscape * landscape =
        StdMutableLandscapeParser::get().parse(path / (name + ".index"));
    StdRestorationPlanParser parser(*landscape);
    RestorationPlan<MutableLandscape> * plan =
        parser.parse(path / (name + ".problem"));

    std::vector<double> budget_values;
    for(double i = 0; i <= plan->totalCost(); i += 1)
        budget_values.push_back(i);
    ...
    for(double budget : budget_values) {
        ...
        for(int i = 0; i < nb_bogo; ++i) {
            ...
            total_bogo_eca +=
                std::pow(eca.eval_solution(*landscape, *plan, *solution), 2);
            delete solution;
        }
        data_log << budget << " " << bogo.toString() << " "
                 << total_bogo_time / nb_bogo << " "
                 << total_bogo_cost / nb_bogo << " " << total_bogo_eca / nb_bogo
                 << " " << std::endl;
        ...
        for(concepts::Solver * solver : solvers) {
            ...
            const double total_eca =
                std::pow(ECA.eval(Helper::decore_landscape(*landscape, *plan,
                                                           *solution)),
                         2);

            data_log << budget << " " << solver->toString() << " "
                     << solution->getComputeTimeMs() << " " << cost << " "
                     << total_eca << " " << std::endl;
